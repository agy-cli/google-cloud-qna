...
# ==========================================
# 1-0. 라우팅 및 필라 에이전트 순수 헬퍼 함수 (Smell 2)
# ==========================================

def _build_routing_prompt(query: str) -> str:
  """라우터 에이전트를 가동하기 위한 프롬프트 문면을 정렬하여 생성한다."""
...
def _parse_routing_response(res_text: str) -> list:
  """라우터 에이전트의 응답 결과 문자열을 파싱하여 유효한 필라 리스트를 반환한다."""
  if not res_text:
...
def _build_pillar_search_query(pillar_name: str) -> str:
  """전문 필라 에이전트의 구글 검색 필터 쿼리를 조합한다."""
  pillar_info = PILLARS[pillar_name]
  filters = pillar_info.get("search_filters", ["cloud.google.com"])
  return " OR ".join([f"site:{f}" for f in filters])

def _build_pillar_prompt(filter_query: str, query: str) -> str:
  """전문 필라 에이전트를 실행하기 위한 프롬프트를 빌드한다."""
  return (
...
def route_inquiry(query: str) -> list:
  ...
  router_prompt = _build_routing_prompt(query)
  try:
    ...
    selected = _parse_routing_response(response.text)
...
def invoke_single_pillar(pillar_name: str, query: str) -> dict:
  ...
  filter_query = _build_pillar_search_query(pillar_name)
  print_thinking(f"서브 에이전트 [{pillar_name}] 가동 및 '{filter_query}' 기반 검색 그라운딩을 동시 수행 중이다.")
  try:
    g_prompt = _build_pillar_prompt(filter_query, query)
    genai_client = get_genai_client()
    response = genai_client.models.generate_content(
...
def generate_technical_advisory(query: str, session_id: str = "default") -> str:
  ...
  url_check_summary = format_url_validation_report(url_validation_results)
  ...
  return normalize_markdown_indentation(final_md)
...
async def route_inquiry_async(client, query: str) -> list:
  ...
  try:
    ...
    return _parse_routing_response(response.text)
  except Exception as e:
...
async def run_orchestrator_sse_async(query: str):
  ...
  final_report_resolved = resolve_all_urls_in_text(final_report)
  final_md = generate_and_upload_diagram(final_report_resolved)
  final_md_cleaned = normalize_markdown_indentation(final_md)
